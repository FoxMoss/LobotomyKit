...
litehtml::web_color Bitmap::get_pixel(int x, int y) const {
  if (x < 0 || x >= width || y < 0 || y >= height)
    return litehtml::web_color::black;
  else
    return data[x + y * width];
}

void Bitmap::set_pixel(int x, int y, litehtml::web_color color) {
  if (x < 0 || x >= width || y < 0 || y >= height)
    return;
  if (color.alpha == 0)
    return;
...
void Bitmap::draw_line(int x0, int y0, int x1, int y1,
...
void Bitmap::draw_rect(int x, int y, int width, int height,
                       litehtml::web_color color) {
...
void Bitmap::fill_rect(litehtml::position rect, litehtml::web_color color) {
...
void Bitmap::replace_color(litehtml::web_color original,
...
litehtml::position Bitmap::find_picture(litehtml::web_color bgcolor) {
...
void Bitmap::resize(int new_width, int new_height) {
  vector<litehtml::web_color> new_data(new_width * new_height,
                                       litehtml::web_color::white);
...
void Bitmap::save(string filename) {
  lodepng::encode(filename, (litehtml::byte *)data.data(), width, height);
}
